...
module.exports = flow = function(temporaryFolder) {
  var $ = this;
  $.temporaryFolder = temporaryFolder + '/tmp/';
  mkdirForFileIfNotExists($.temporaryFolder);
  ...
  $.post = function(req, fields, resourcePath, callback) {
    ...
    var validation = validateRequest(chunkNumber, chunkSize, totalSize, identifier, filename, fields['flowCurrentChunkSize']);
    if (validation == 'valid') {
      var chunkFilename = getChunkFilename(chunkNumber, identifier);
